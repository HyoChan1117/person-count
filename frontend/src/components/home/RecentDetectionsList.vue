<template>
  <UiCard class="flex min-h-0 flex-1 flex-col">
    <SectionHeader title="최근 감지 로그" :description="`${scope} · 얼굴은 기본 블러 · 클릭하면 공개`" />
    ...
    <!-- 확대 보기는 body로 보내 카드 바깥에 띄운다(루트를 하나로 유지해야 부모가 준 class가 카드에 전달된다) -->
    <Teleport to="body">
      <div
        v-if="preview"
...
defineProps({
  ...
  scope: { type: String, default: '전체 교실' }, // 순찰 상태 카드에서 고른 교실 이름
})
